...
from update.process import convert_nix_hash_to_sri, run_command, run_nix_build

if TYPE_CHECKING:
    from collections.abc import Iterable, Mapping

    from update.updaters.base import CargoLockGitDep
...
def _extract_nix_hash(output: str, *, config: UpdateConfig | None = None) -> str:
    ...
    if has_mismatch_signal:
        msg = (
            ...
            "libnix.commands.base.HashMismatchError. Output tail:\n"
...
def _build_overlay_expr(source: str, *, system: str | None = None) -> str:
    ...
    """
    system_arg = f'"{system}"' if system else "builtins.currentSystem"
    return _compact_nix_expr(
        f'let flake = builtins.getFlake "git+file://{get_repo_file(".")}?dirty=1"; '
...
async def compute_overlay_hash(
    ...
    *,
    ...
    The overlay definition in ``overlays.nix`` is the single source of truth.
    """
...
def _build_deno_deps_expr(source: str, platform: str) -> str:
    ...
    with a temporary ``sources.json`` (via ``SOURCES_JSON`` env var).
    """
...
async def compute_bun_node_modules_hash(
    source: str,
    _input_name: str = "",
    *,
    config: UpdateConfig | None = None,
