"""Assemble and verify Buzz's pinned Mesh native runtime bundle."""
...
_CLI_ARGUMENT_COUNT = 12


def fail(message: str) -> Never:
    """Stop packaging with the causal validation diagnostic."""
    raise SystemExit(message)


def sha256(path: Path) -> str:
    """Hash one regular runtime or provenance input."""
    return hashlib.sha256(path.read_bytes()).hexdigest()


def load_json(path: Path, label: str) -> object:
    """Decode a labeled input before validating its domain shape."""
    try:
...
def safe_relative_path(value: object, label: str) -> PurePosixPath:
    """Reject absolute, traversing, or noncanonical manifest paths."""
...
def require_regular_file(path: Path, label: str) -> None:
    """Reject symlinks and missing files at provenance boundaries."""
    if path.is_symlink() or not path.is_file():
        fail(f"{label} is not a regular file: {path}")


def validate_digest(value: object, label: str) -> None:
    """Require the manifest digest encoding used by the runtime loader."""
...
def validate_mesh_provenance(
    ...
    """Check source identity and every attested patch and packaging input."""
...
def is_dylib_name(name: str) -> bool:
    """Recognize dylib filenames and versioned dylib suffixes."""
    return name.endswith(".dylib") or ".dylib." in name


def is_within(relative: str, parent: str) -> bool:
    """Compare canonical relative paths at component boundaries."""
    return relative == parent or relative.startswith(f"{parent}/")


def validate_library_symlink(path: Path, library_root: Path) -> None:
    """Allow only local aliases to physical runtime libraries."""
    target = os.readlink(path)  # noqa: PTH115 -- raw target spelling is validated before normalization
...
def validate_llama_inventory(
    llama_root: Path, lib_subdir: str, resource_values: object
) -> tuple[Path, list[PurePosixPath], list[Path]]:
    """Validate the library closure and declared nonlibrary resources."""
    if lib_subdir != "lib":
...
def copy_runtime(
    ...
    """Copy the verified runtime closure and preserve its aliases and bytes."""
    ...
    for relative in resource_paths:
        ...
        if sha256(source) != sha256(
            destination
        ):  # pragma: no cover -- post-copy filesystem corruption defense
    ...
    for source in sorted(library_root.rglob("*"), key=lambda path: path.as_posix()):
        ...
        if source.is_symlink():
            if (
                ...
                or destination.readlink() != source.readlink()
            ):  # pragma: no cover -- post-copy filesystem corruption defense
...
def manifest_files(output_root: Path) -> dict[str, str]:
    """Digest the installed runtime files, excluding the manifest itself."""
    files = {}
...
def validate_manifest_files(files: dict[str, str], libraries: list[str]) -> None:
    """Require every loaded library to have an attested file digest."""
...
def run_tool(command: list[str], label: str) -> tuple[str, str]:
    """Capture a native inspection tool result or report its causal failure."""
    result = subprocess.run(  # noqa: S603 -- explicit native tool arguments
        command,
        check=False,
        capture_output=True,
        text=True,
...
def resolve_dependency(
    ...
    library: Path,
    library_root: Path,
    physical_by_path: dict[Path, str],
) -> str | None:
    """Resolve a local runtime edge or allow a macOS system library."""
    ...
    if (
        canonical is None
    ):  # pragma: no cover -- complete physical inventory makes this unreachable
        fail(f"unresolved local dependency for {library.name}: {dependency}")
    return canonical


def inspect_libraries(library_root: Path, nm: str, codesign: str) -> list[str]:
    """Check architecture, signatures, symbols, and dependency load order."""
    physical_libraries = sorted(
        (
            path
            for path in library_root.rglob("*")
    ...
    physical_by_path = {
        path.resolve(strict=True): relative for path, relative in canonical.items()
    }
    ...
    for library in physical_libraries:
        ...
        if metadata.install_id is None:
            fail(f"expected one install ID for {library.name}")
        if metadata.install_id != f"@rpath/{library.name}":
            fail(f"unexpected install ID for {library.name}: {metadata.install_id}")
...
def main(arguments: list[str]) -> None:
    """Build the bundle from pinned identities and explicit native tools."""
    if len(arguments) != _CLI_ARGUMENT_COUNT:
        fail("bundle script requires exactly twelve arguments")
    ...
    nm, codesign = arguments[6:8]
    mesh_version, mesh_commit, llama_commit, skippy_abi = arguments[8:]

    validate_mesh_provenance(
        mesh_source, mesh_provenance, mesh_version, mesh_commit, llama_commit
    )
    ...
    libraries = inspect_libraries(output_library_root, nm, codesign)
