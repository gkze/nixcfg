...
patch_opencode_desktop_source() {
  ...
  if [ -f "$desktop_package_path/src/main/lifecycle/environment.ts" ]; then
    # The filtered install does not expose the release helper to this script.
    substituteInPlace "$desktop_package_path/scripts/prepare.ts" \
      --replace-fail 'import { Script } from "@opencode/script"' \
      'const Script = { version: process.env.OPENCODE_VERSION ?? "0.0.0" }'

    # Every packaged channel uses the Nix-built sidecar, including dev.
    substituteInPlace "$desktop_package_path/scripts/prebuild.ts" \
      --replace-fail 'if (channel === "dev") await downloadCliToResources()' \
      'if (channel === "dev") await copyBuiltCliToResources(Bun.env.OPENCODE_CLI_DIST!)'

    substituteInPlace "$desktop_package_path/src/main/lifecycle/environment.ts" \
      --replace-fail 'const appID = app.isPackaged ? appIDs[CHANNEL] : "ai.opencode.desktop.dev"' \
      'const appID = process.env.OPENCODE_APP_ID ?? (app.isPackaged ? appIDs[CHANNEL] : "ai.opencode.desktop.dev")' \
      --replace-fail 'app.setName(app.isPackaged ? appNames[CHANNEL] : "OpenCode Dev")' \
      'app.setName(process.env.OPENCODE_APP_NAME ?? (app.isPackaged ? appNames[CHANNEL] : "OpenCode Dev"))' \
    ...
    substituteInPlace "$desktop_package_path/src/main/lifecycle/index.ts" \
      --replace-fail 'const urls = argv.filter((arg) => arg.startsWith("opencode://"))' \
      'const urls = argv.filter((arg) => arg.startsWith((process.env.OPENCODE_PROTOCOL_SCHEME ?? "opencode") + "://"))'
    substituteInPlace "$desktop_package_path/electron-builder.config.ts" \
      --replace-fail 'export default getConfig()' \
      'export default { ...getConfig(), protocols: { name: process.env.OPENCODE_PROTOCOL_NAME ?? "OpenCode", schemes: [process.env.OPENCODE_PROTOCOL_SCHEME ?? "opencode"] } }'
  else
