...
from libnix.models.hash import HashAlgorithm, NixHash, is_sri, make_sri, parse_sri
from libnix.models.sources import SourcesFile
from libnix.models.store_path import (
    StorePath,
    full_path,
    is_derivation,
    parse_store_path,
...
class TestSourcesFile:
    """Loading and round-tripping sources.json."""

    @pytest.fixture
    def sources(self) -> SourcesFile:
        """Load the real sources.json from the project root."""
        assert SOURCES_JSON_PATH.exists(), (
            f"sources.json not found at {SOURCES_JSON_PATH}"
        )
        return SourcesFile.load(SOURCES_JSON_PATH)

    def test_sources_file_load(self, sources: SourcesFile) -> None:
        """Load the actual sources.json; verify it has entries."""
