...
def _load_normalizer_module() -> ModuleType:
    module_path = REPO_ROOT / "overlays" / "goose-cli" / "normalize_cargo_nix.py"
...
def test_normalize_is_noop_for_checked_in_goose_cargo_nix() -> None:
    ...
    module = _load_normalizer_module()
    cargo_nix = REPO_ROOT / "overlays" / "goose-cli" / "Cargo.nix"
...
def test_bootstrap_repo_import_path_prefers_env_root(
    ...
    module = _load_normalizer_module()
    monkeypatch.setenv("REPO_ROOT", "/tmp/goose-root")
    monkeypatch.setattr(module.sys, "path", [])

    module._bootstrap_repo_import_path()

    assert module.sys.path == [str(Path("/tmp/goose-root").resolve())]
...
def test_bootstrap_repo_import_path_finds_root_marker_and_errors_when_missing(
    ...
    """Repo-root discovery should walk candidates and fail clearly without a marker."""
    module = _load_normalizer_module()
    repo_root = tmp_path / "repo"
    nested = repo_root / "overlays" / "goose-cli"
...
def test_resolve_path_handles_relative_and_absolute_paths(
    ...
    assert module._resolve_path("overlays/goose-cli/Cargo.nix") == Path(
        "/repo/overlays/goose-cli/Cargo.nix"
    )
...
def test_main_uses_default_path_and_reports_no_change(
    ...
    """Main should default to the checked-in overlay Cargo.nix path without rewriting."""
    module = _load_normalizer_module()
    repo_root = tmp_path / "repo"
    cargo_nix = repo_root / "overlays" / "goose-cli" / "Cargo.nix"
...
def test_main_guard_exits_with_main_result(
    ...
    script_path = REPO_ROOT / "overlays/goose-cli/normalize_cargo_nix.py"
