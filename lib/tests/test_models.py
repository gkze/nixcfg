...
class TestFlakeLock:
    ...
    def test_flake_lock_get_locked_path_resolution_edges(self) -> None:
        ...
        locked = expect_not_none(locked)
        assert locked.nar_hash == sri
        assert lock.get_locked("missingNode") is None
        assert lock.get_locked("badRef") is None
        assert lock.get_locked("ghostFinal") is None
    ...
    def test_flake_lock_path_resolution_handles_none_node_name(self) -> None:
        ...
        assert lock.get_locked("broken") is None
...
class TestDerivation:
    ...
    def test_derivation_output_helper_properties(self) -> None:
        ...
        assert no_hash.output_names == ["dev", "out"]
        assert no_hash.is_fixed_output is False
        ...
        assert with_hash.is_fixed_output is True
...
class TestSourcesFile:
    ...
    def test_hash_collection_parsing_and_primary_hash_variants(self) -> None:
        ...
        assert entries_collection.to_json() == [entry.to_dict()]
        assert entries_collection.primary_hash() == h1

        mapping_collection = HashCollection.from_value({"darwin": h2, "linux": h2})
        assert mapping_collection.to_json() == {"darwin": h2, "linux": h2}
        assert mapping_collection.primary_hash() == h2
    ...
    def test_hash_collection_parse_input_additional_paths(self) -> None:
        ...
        assert parsed == {"entries": None, "mapping": {"linux": h1}}

        distinct_mapping = HashCollection.from_value({"darwin": h1, "linux": h2})
        assert distinct_mapping.primary_hash() is None
    ...
    def test_source_entry_sources_file_merge_load_save(
        ...
        assert merged_entry.version == "2.0.0"
        assert merged_entry.input == "incoming-input"
        assert merged_entry.to_dict()["drvHash"] == "drv-incoming"
        assert merged_entry.urls == {
            "mirror": "https://example.invalid/mirror.tar.gz",
        ...
        assert set(merged_file.entries) == {"pkgA", "pkgB"}
        assert merged_file.entries["pkgA"].version == "2.0.0"
    ...
    def test_sources_file_json_schema_structure(self) -> None:
        ...
        assert first_variant["type"] == "array"
