...
def test_normalize_is_noop_for_checked_in_zed_cargo_nix() -> None:
    ...
    cargo_nix = REPO_ROOT / "packages" / "zed-editor-nightly" / "Cargo.nix"
...
def test_bootstrap_repo_import_path_prefers_env_root(
    ...
    module = _load_normalizer_module()
    monkeypatch.setenv("REPO_ROOT", "/tmp/zed-root")
    monkeypatch.setattr(module.sys, "path", [])

    module._bootstrap_repo_import_path()

    assert module.sys.path == [str(Path("/tmp/zed-root").resolve())]
...
def test_bootstrap_repo_import_path_finds_root_marker_and_errors_when_missing(
    ...
    nested = repo_root / "packages" / "zed-editor-nightly"
...
def test_resolve_path_handles_relative_and_absolute_paths(
    ...
    assert module._resolve_path("packages/zed-editor-nightly/Cargo.nix") == Path(
        "/repo/packages/zed-editor-nightly/Cargo.nix"
    )
...
def test_main_updates_file_and_reports_status(
    ...
    monkeypatch.setattr(
        ...
        "normalize",
        lambda text: (
            ("normalized", 1, True) if text == "original" else (text, 0, False)
        ),
    )
...
def test_main_guard_exits_with_main_result(
    ...
    script_path = REPO_ROOT / "packages/zed-editor-nightly/normalize_cargo_nix.py"
