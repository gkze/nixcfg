...
def test_phase_resolve_success_and_failure(
    ...
    assert object.__getattribute__(pipeline, "_phase_resolve")(pinned)

    monkeypatch.setattr("lib.update.ci.resolve_versions.main", lambda _argv: 2)
    assert not object.__getattribute__(pipeline, "_phase_resolve")(pinned)
...
def test_phase_compute(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert object.__getattribute__(pipeline, "_phase_compute")(pinned)

    monkeypatch.setattr("lib.update.cli.run_updates", lambda _opts: 1)
    assert not object.__getattribute__(pipeline, "_phase_compute")(
        pinned, source="demo"
    )
...
def test_split_sources_and_merge_phase(
    ...
    assert set(roots) == set(pipeline.CI_PLATFORMS)
    ...
    assert object.__getattribute__(pipeline, "_phase_merge")(work)

    monkeypatch.setattr("lib.update.ci.merge_sources.main", lambda _args: 1)
    assert not object.__getattribute__(pipeline, "_phase_merge")(work)
...
def test_phase_validate(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert object.__getattribute__(pipeline, "_phase_validate")()

    monkeypatch.setattr(
        pipeline, "package_file_map", lambda _name: {"good": good, "bad": bad}
    )
    assert not object.__getattribute__(pipeline, "_phase_validate")()
...
def test_main_parses_typer_options(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert called["resolve_only"] is True
    assert called["keep_artifacts"] is True
...
def test_print_summary(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert removed == [work]
    assert any("Pipeline FAILED" in line for line in logs)

    logs.clear()
    work2 = tmp_path / "work2"
    work2.mkdir()
    object.__getattribute__(pipeline, "_print_summary")([("a", True)], work2, keep=True)
    assert any("Artifacts kept" in line for line in logs)
...
def test_run_and_print_summary_remaining_branch_paths(
    ...
    assert any("All phases passed" in line for line in logs)
...
def test_phase_crate2nix_logs_success_and_failure(
    ...
    assert pipeline._phase_crate2nix() is True
    assert any("crate2nix freshness OK" in line for line in logs)

    logs.clear()
    monkeypatch.setattr(pipeline.crate2nix, "main", lambda _args: 9)
    assert pipeline._phase_crate2nix() is False
    assert any("pipeline crate2nix exited 9" in line for line in logs)
