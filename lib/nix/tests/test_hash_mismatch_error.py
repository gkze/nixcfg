...
from lib.nix.tests._assertions import check, expect_not_none
...
class TestHashMismatchError:
    ...
    def test_fod_sha256_sri(self) -> None:
        """Run this test case."""
        result = object.__getattribute__(self, "_make_result")(self.SAMPLE_FOD_STDERR)
        err = HashMismatchError.from_output(self.SAMPLE_FOD_STDERR, result)
        err = expect_not_none(err)
        check(err.hash == "sha256-ungWv48Bz+pBQUDeXa4iI7ADYaOWF3qctBD/YfIAFa0=")
        check(err.specified == "sha256-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=")
        check(err.drv_path == "/nix/store/g1w7hy3qg1w7hy3qg1w7hy3qg1w7hy3q-source.drv")
    ...
    def test_fod_sha512_sri(self) -> None:
        ...
        err = expect_not_none(err)
        check(err.hash == "sha512-BBBB=")
        check(err.specified == "sha512-AAAA=")
    ...
    def test_fod_sha1_sri(self) -> None:
        ...
        err = expect_not_none(err)
        check(err.hash == "sha1-BBBB=")
        check(err.specified == "sha1-AAAA=")

    def test_nar_import_nix32(self) -> None:
        """Run this test case."""
        err = HashMismatchError.from_output(
            self.SAMPLE_NAR_MISMATCH,
            object.__getattribute__(self, "_make_result")(self.SAMPLE_NAR_MISMATCH),
        )
        err = expect_not_none(err)
        check(err.hash == "1d6b9xw51a1289ymqaax76ra2gi2i3297kdd4q5sxjaxhicnmwal")
        check(err.specified == "0c5b8vw40d1178xlpddw65q9gf1h2186jcc3p4swinwggbllv8mk")
        check(err.drv_path == "/nix/store/abc-foo")

    def test_ca_import_nix32(self) -> None:
        """Run this test case."""
        err = HashMismatchError.from_output(
            self.SAMPLE_CA_MISMATCH,
            object.__getattribute__(self, "_make_result")(self.SAMPLE_CA_MISMATCH),
        )
        err = expect_not_none(err)
        check(err.hash == "1d6b9xw51a1289ymqaax76ra2gi2i3297kdd4q5sxjaxhicnmwal")
        check(err.drv_path == "/nix/store/def-bar")

    def test_prefixed_hex(self) -> None:
        """Run this test case."""
        err = HashMismatchError.from_output(
            self.SAMPLE_HEX_MISMATCH,
            object.__getattribute__(self, "_make_result")(self.SAMPLE_HEX_MISMATCH),
        )
        err = expect_not_none(err)
        check(
            err.hash
    ...
    def test_nested_derivation_takes_last_match(self) -> None:
        ...
        err = expect_not_none(err)
        check(err.hash == "sha256-INNERgotxINNERgotxINNERgotxINNERgotxINNERgo0=")
        check(err.specified == "sha256-INNERspecINNERspecINNERspecINNERspecINNERspe0=")
    ...
    def test_hash_in_stdout_not_stderr(self) -> None:
        ...
        err = HashMismatchError.from_output(
            stdout,
            object.__getattribute__(self, "_make_result")(stdout="", stderr=""),
        )
        err = expect_not_none(err)
        check(err.hash == "sha256-BBB=")
    ...
    def test_whitespace_variations(self) -> None:
        ...
        err = expect_not_none(err)
        check(err.hash == "sha256-BBBB=")
        check(err.specified == "sha256-AAAA=")

    def test_no_match_returns_none(self) -> None:
        """Run this test case."""
        unrelated = "error: attribute 'foo' missing\n"
        err = HashMismatchError.from_output(
            unrelated, object.__getattribute__(self, "_make_result")(unrelated)
        )
        check(err is None)
    ...
    def test_base64_with_plus_and_slash(self) -> None:
        ...
        err = expect_not_none(err)
        check(err.hash == "sha256-a+b/c+D/eF+gH/iJ+kL/mN+oP/qR+sT/uV+wX/yZ0=")
    ...
    def test_no_drv_path_still_extracts_hash(self) -> None:
        ...
        err = expect_not_none(err)
        check(err.hash == "sha256-NEW=")
        check(err.specified == "sha256-OLD=")
        check(err.drv_path is None)
